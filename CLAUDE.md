...
**`page_images.py` keeps the rulebook's art local.** It owns one decision — what an
`<img src>` on a given page resolves to — and one transform: rewriting the tag to an
inline `data:` URI from the `images` table. Both `app.py` (reading) and
`scripts/build_images.py` (fetching) call the *same* `resolve`, so the key a page asks
for and the key the fetcher stored cannot drift; that is the whole reason the resolver
is a shared function rather than a line of string-joining at each call site. It takes a
`lookup` callable instead of a connection, which keeps it Qt-free *and* SQL-free.
Note the corpus quirks it exists to absorb: 66 tags use Windows separators, and some
srcs climb out of one book into another (`..\AEG\DD90000.gif` on an `HLC/` page).
...
- **`dnd2e.db`** (~74 MB) is the scraped rulebook DB, committed to the repo and
...
- **The app makes no network requests.** That is a property worth keeping. It was
  briefly untrue by accident: the scrape took the rulebook's text but not its
  pictures, and because the web view resolves relative URLs against `BASE_URL`,
  every `<img>` in a stored page was a silent live fetch from regalgoblins.com.
  The art (317 files, ~10 MB) now lives in the DB's `images` table and
  `page_images.py` rewrites each tag to an inline `data:` URI. The only remaining
  outbound call is `rules_agent` → **localhost** Ollama. If you add a feature that
  reaches the network at runtime, that is a design change, not a detail.
...
- Some modules are **generated — do not hand-edit**. They say so at the top:
  ...
  - spell data ← `scripts/build_spells.py`; economics ← `scripts/build_economics.py`;
    ...
    the `images` table ← `scripts/build_images.py` (page art, from the site)
  ...
  `build_images.py` is idempotent — it fetches only what the `images` table is
  missing, so a re-run after a re-scrape downloads just the new art.
