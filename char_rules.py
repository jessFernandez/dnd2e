...
# Rules prose for the CT fighting styles, unarmed disciplines and talents, so the
# builder can show a "What it does" block. Generated by scripts/build_ct_text.py;
# absent in a stripped checkout, in which case the blocks simply don't render.
try:
    from ct_text import CT_TEXT
except ImportError:                                   # pragma: no cover
    CT_TEXT = {}


def ct_description(name: str) -> str:
    """The rulebook's text for a style / discipline / talent, '' if unknown."""
    return CT_TEXT.get(name, {}).get("description", "")


def ct_page(name: str) -> str:
    """The Combat & Tactics page it came from, '' if unknown."""
    return CT_TEXT.get(name, {}).get("page", "")
