...
from ask_retrieval import linkify, mark_house_rules
...
def test_linkify_bare_url_in_prose():
    out = linkify("see dnd:///PHB/DD01422.htm for details", TITLES)
...
def test_linkify_preserves_existing_link():
    ...
    assert linkify(src, TITLES) == src


def test_linkify_unknown_url_falls_back_to_filename():
    out = linkify("[dnd:///XYZ/DD99999.htm]", {})
    assert "[DD99999](dnd:///XYZ/DD99999.htm)" in out


def test_mark_house_rules_label():
    assert mark_house_rules("House rule: no critical hits.").startswith("⚔️ **House Rule:**")


def test_mark_house_rules_inline_citation():
    assert "(⚔️ house rule)" in mark_house_rules("Damage is capped (house rule).")


def test_mark_house_rules_idempotent():
    once = mark_house_rules("House Rule: x")
    assert mark_house_rules(once) == once
